...
new_battery_value = float(input(f"Enter new battery system value (default {new_battery_value_default:.2f}) ").strip() or
...
battery_install_cost = float(input(f"Enter your battery installation cost (default {battery_install_cost_default:.2f}) ") or
...
for year in range (1,21,1):
    print(f"Year {year}:")
    print(f"    Possible this year's savings: {annual_possible_savings:.2f}")
    total_possible_savings += annual_possible_savings
    print(f"    Possible total electrical savings: {total_possible_savings:.2f}")
    ...
    print(f"    No battery additional electrical cost for the year ${annual_no_battery:.2f} ")
    ...
    print(f"    Current Battery value: {new_battery_value}")

    electrical_savings_investment += total_possible_savings * investment_annual_return
    print(f"    electrical savings earnings if invested {electrical_savings_investment:.2f}")

    print("")

    if (total_possible_savings + electrical_savings_investment) >= battery_install_cost:
        break

print(f"Total possible savings {total_possible_savings:.2f}")
total_possible_savings += electrical_savings_investment
print(f"Total possible savings if invested: ${total_possible_savings:.2f}")

opportunity_cost -= battery_install_cost
print(f"Opportunity cost if you didn't buy a battery but invested the money. ${opportunity_cost:.2f}")

print(f"But total cost of electricity without a battery ${total_no_battery_cost:.2f}")
